...
function ShowDemoWindow()
    ...
    if CImGui.CollapsingHeader("Configuration")
        ...
        aaidx = findfirst(x -> x == :AntiAliasedLines, fieldnames(ImPlot.ImPlot_ImPlotStyle))
        aaoffset = fieldoffset(ImPlot.ImPlot_ImPlotStyle, aaidx)
    ...
    if CImGui.CollapsingHeader("Time Formatted Axes")
        ...
        begin 
            if @c CImGui.Checkbox("Local Time", &use_local_time)
                CImGui.Set(ImPlot.ImPlot_GetStyle(), :UseLocalTime, use_24hour_clock)
            end
            CImGui.SameLine()
            if @c CImGui.Checkbox("ISO 8601", &use_ISO8601)
                CImGui.Set(ImPlot.ImPlot_GetStyle(), :UseISO8601, use_ISO8601)
            end
            CImGui.SameLine()
            if @c CImGui.Checkbox("24 Hour Clock", &use_24hour_clock)
                CImGui.Set(ImPlot.ImPlot_GetStyle(), :Use24HourClock, use_24hour_clock)
            end
        end)
    ...
    if CImGui.CollapsingHeader("Legend")
        ...
        begin
            ...
            if CImGui.SliderFloat2("LegendInnerPadding", Ptr{Float32}(pointer_from_objref(inner_padding)), 0.0, 10.0, "%.0f")
                CImGui.Set(ImPlot.ImPlot_GetStyle(), :LegendInnerPadding, inner_padding[])
            end
            spacing = Ref(style.LegendSpacing)
            if CImGui.SliderFloat2("LegendSpacing", Ptr{Float32}(pointer_from_objref(spacing)), 0.0, 5.0, "%.0f")
                CImGui.Set(ImPlot.ImPlot_GetStyle(), :LegendSpacing, spacing[])
            end
